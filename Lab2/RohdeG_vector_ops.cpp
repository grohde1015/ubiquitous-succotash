...
std:vector<float> vectorstuff::vectors::subtract_arrays(std::vector<float> *array1, int size1, std::vector<float> *array2){
    ...
    for(i=0;i<size1;i++){
        subtract_arrays.push_back(0);
    }

    for(j=0;j<size1;j++){
        subtract_arrays[j] = ((*array1)[j] - (*array2)[j]);
    }

    return subtract_arrays; 
}
std::vector<float> vectorstuff::vectors::divide(std::vector<float> *data, int size, int divider){
